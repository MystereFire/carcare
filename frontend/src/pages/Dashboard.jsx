...
export default function Dashboard() {
  ...
  const [selectedId, setSelectedId] = useState(() => {
    const val = id || localStorage.getItem('currentVehicleId') || '';
  ...
  const handleQuickOdometerUpdate = async () => {
    if (!selectedVehicle) return;
    const newKm = window.prompt("Entrez le nouveau kilométrage du véhicule :", currentKm);
    if (newKm === null) return;
    const kmNum = parseFloat(newKm);
    if (isNaN(kmNum) || kmNum < 0) {
      alert("Veuillez entrer un nombre valide supérieur ou égal à 0.");
      return;
    }
    if (kmNum < currentKm) {
    ...
    try {
      await api.put(`/api/vehicles/${selectedVehicle._id}`, { currentOdometer: kmNum });
      setVehicles(prev => prev.map(v => v._id === selectedVehicle._id ? { ...v, currentOdometer: kmNum } : v));
      // Refresh the page data to reload expenses and maintenance tasks status which depend on current mileage
      await Promise.all([
        fetchVehicles(),
        loadExpenses(selectedVehicle._id)
      ]);
    } catch (err) {
      console.error(err);
    ...
    <>
      <PageTransition>
        <div className="min-h-screen bg-[#070a13] text-slate-100">
            ...
            <aside className="hidden lg:flex w-72 flex-col m-4 rounded-3xl bg-slate-900/60 backdrop-blur-2xl border border-white/5 shadow-2xl sticky top-4 h-[calc(100vh-2rem)]">
              ...
              <nav className="flex-1 px-4 space-y-2 mt-2 flex flex-col">
                ...
                <button
                  ...
                  className="hidden w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-slate-400 hover:bg-white/5 hover:text-slate-200 transition-all duration-200 font-medium"
                  disabled={!selectedId}
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button
                  ...
                  disabled={!selectedId}
                  className="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-slate-400 hover:bg-white/5 hover:text-slate-200 transition-all duration-200 font-medium disabled:opacity-50"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {!isSingleVehicle && (
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-slate-400 hover:bg-white/5 hover:text-slate-200 transition-all duration-200 font-medium"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button
                  ...
                  className="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-red-400 hover:bg-red-950/20 hover:text-red-300 transition-all duration-200 font-medium mt-auto"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                      strokeLinecap="round"
            ...
            <div className="flex-1 flex flex-col h-[calc(100vh-2rem)] overflow-y-auto scrollbar-hide">
              <header className="px-8 py-6 flex flex-col md:flex-row md:items-center md:justify-between sticky top-0 z-10 bg-slate-900/80 backdrop-blur-md border-b border-white/5 mb-6 rounded-b-3xl lg:rounded-none lg:bg-transparent lg:border-none lg:backdrop-blur-none lg:static">
                <div>
                    ...
                    {selectedVehicle ? (
                      <>
                        ...
                        {selectedVehicle.year ? (
                          <span className="text-sm font-semibold text-slate-300 bg-white/5 px-3 py-1 rounded-full border border-white/10 shadow-sm backdrop-blur-sm">
                            {selectedVehicle.year}
                          </span>
                        ) : null}
                      </>
                  ...
                  <p className="text-sm text-slate-400 mt-2 flex flex-wrap gap-3 items-center font-medium">
                    {selectedVehicle?.plate ? <span className="px-2 py-0.5 bg-slate-900/60 rounded text-slate-300 border border-white/10">Immat: {selectedVehicle.plate}</span> : null}
                    {selectedVehicle?.brand || selectedVehicle?.model ? (
                      ...
                      <>
                        ...
                        <span
                           ...
                           className={clsx(
                             ...
                             new Date(selectedVehicle.technicalInspectionDate) < new Date() ? "bg-red-950/20 text-red-400 border-red-500/20" : "bg-green-950/20 text-green-400 border-green-500/20"
                           )}
                        >
              ...
              <div className="px-8 pb-8 space-y-8">
                ...
                {loadingVehicles ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-24 text-center glass-card rounded-3xl shadow-xl border border-white/5">
                    ...
                    <p className="text-xl font-bold text-slate-100">Ajoutez votre premier véhicule</p>
                  ...
                  <>
                        ...
                        <div className="z-10 flex flex-col justify-between h-full space-y-8">
                          <div>
                            ...
                            <div className="flex flex-wrap items-baseline gap-3">
                              <p className="text-6xl font-black text-slate-100 tracking-tighter speed-font">
                                {formatNumber(currentKm)}
                                <span className="text-2xl text-slate-500 font-medium ml-2">km</span>
                              </p>
                              <button
                            ...
                            <div className="mt-4 inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-emerald-950/20 text-emerald-400 border border-emerald-500/20 text-sm font-semibold">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      ...
                      <div className="glass-card p-8 rounded-3xl flex flex-col items-center justify-center glass-card-hover">
                        ...
                        <div className="relative w-56 h-56">
                          <div className="w-full h-full rounded-full shadow-xl ring-4 ring-slate-950/40" style={donutSegments} />
                          <div className="absolute inset-6 bg-slate-900/90 backdrop-blur-sm rounded-full flex flex-col items-center justify-center shadow-inner border border-white/5">
                            <span className="text-3xl font-black text-slate-100 tracking-tight speed-font">
                        ...
                        <div className="mt-8 w-full space-y-4">
                          {[
                            ...
                            <div key={item.key} className="flex justify-between items-center text-sm p-2 rounded-lg hover:bg-white/5 transition-colors">
                              <div className="flex items-center gap-3">
                                <span className={clsx('w-3 h-3 rounded-full ring-2 ring-slate-950 shadow-sm', item.color)} />
                                <span className="text-slate-400 font-medium">{item.label}</span>
                              </div>
                              <span className="font-bold text-slate-200 bg-white/5 px-2 py-1 rounded-md text-xs">
                                {totals.total
                    ...
                    <div className="glass-card rounded-3xl overflow-hidden border border-white/5">
                      ...
                      {expensesError ? (
                        <div className="p-8 text-sm text-amber-300 bg-amber-950/20 border-t border-white/5">
                          {expensesError}
                        </div>
                      ) : loadingExpenses ? (
                        <div className="p-8 animate-pulse text-slate-400">Chargement des dépenses...</div>
                          ...
                          <table className="w-full text-left text-sm">
                            ...
                            <tbody className="divide-y divide-white/5">
                              {recentExpenses.map((exp) => (
                                <tr key={exp._id} className="hover:bg-white/5 transition-colors group cursor-default">
                                  ...
                                  <td className="px-8 py-5 text-slate-400">
                                    {exp.km ? <span className="font-mono text-slate-300 bg-white/5 px-2.5 py-1 rounded-lg border border-white/5 text-xs">{formatNumber(exp.km)} km</span> : <span className="italic text-slate-500">{exp.notes || '—'}</span>}
                                  </td>
                                  <td className="px-8 py-5 text-right font-bold text-slate-100">
                      ...
                      <div className="space-y-6">
                        <div className="flex items-center justify-between px-2">
                          <div>
                            ...
                            <h3 className="text-2xl font-black text-slate-100">Sélection rapide</h3>
                          </div>
                          <button
                            onClick={handleAddVehicle}
                            ...
                            return (
                              ...
                              >
                                <div className="relative h-48 overflow-hidden">
                                  <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/20 to-transparent z-10 opacity-95" />
                                  <img
                                    src={imgSrc}
          ...
          {showExpenseModal && (
            <AddExpense vehicleId={selectedId} onClose={() => { setShowExpenseModal(false); fetchVehicles(); loadExpenses(selectedId); }} />
          )}
        </div>
      </PageTransition>
    </>
